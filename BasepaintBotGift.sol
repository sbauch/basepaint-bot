...
contract BasePaintBot is Ownable, ERC721 {
    ...
    event BalanceLessThanRequired(
        uint160 indexed _address,
        uint96 indexed _actualBalance,
        uint256 indexed _expectedBalance
    );

    event FeeUpdated(uint256 indexed newFeeBasisPoints);

    event MintSkippedBalance(
        uint160 indexed _address,
        uint96 indexed _balance,
        uint16 indexed _amountToMint
    );

    event MintSkippedDaily(
        uint160 indexed _address,
        uint96 indexed _balance,
        uint16 indexed _lastMinted
    );

    event Subscribed(
        address indexed subscriber,
        address indexed mintTo,
        uint96 indexed balance,
        uint16 mintPerDay
    );

    event SubscriptionGifted(
        address indexed giftedFrom,
        address indexed giftedTo,
        uint96 indexed balance,
        uint16 mintPerDay
    );
    ...
    struct Subscription {
        ...
        uint96 balance;
        // How many tokens to mint per day (max 65,535)
        uint16 mintPerDay;
    ...
    function subscribe(
        ...
        uint160 mintTo = _mintToAddress == address(0)
            ? uint160(sender)
            : uint160(_mintToAddress);
    ...
    function giftASubscription(
        uint16 _mintPerDay,
        address _giftSubscriptionTo,
        uint256 _length
    ) external payable {
        uint256 value = msg.value;
        if (value == 0) revert MustSendETH();
        ...
        address owner = _giftSubscriptionTo;
        ...
        // If you need to top up an existing Subscription,
        // use the `receive` function
        Subscription storage sub = subscriptions[owner];
        if (sub.mintToAddress != 0) revert SubAlreadyExists();

        uint96 balance = uint96(value);

        sub.mintToAddress = uint160(owner);
        sub.balance = balance;
        sub.mintPerDay = _mintPerDay;
        sub.lastMinted = 0;
        sub.owner = uint160(owner);

        _mint(owner, this.packTokenId(owner, uint96(basepaint.today())));

        emit SubscriptionGifted(msg.sender, owner, balance, _mintPerDay);
    }
    ...
    function mintDaily(
        ...
        for (uint256 i; i < _addresses.length; ) {
            ...
            if (subscription.lastMinted == today) {
                emit MintSkippedDaily(
                    subscription.owner,
                    subscription.balance,
                    subscription.lastMinted
                );
            ...
            basepaint.safeTransferFrom(
                address(this),
                address(subscription.mintToAddress),
                today,
                count,
                ""
            );
    ...
    function setNewBasepaintAPIProxy(
    ...
    function setNewBasepaintAPIBase(
